// Package audit — тонкая обёртка вокруг packages/audit-sdk для client-service.
// Lazy singleton: клиент инициализируется один раз через sync.Once и
// переиспользуется во всех handler-middleware.
//
// Интеграция следует ADR-0010: смена статуса клиента и иные мутации
// фиксируются в append-only audit log сервиса audit-service, чтобы биллинг
// и операционные отчёты могли по correlation_id связать audit-запись с
// биллинговой строкой.
package audit
