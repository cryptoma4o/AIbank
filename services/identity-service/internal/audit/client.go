...
// identity-service.  Lazy singleton: клиент инициализируется один раз
// через sync.Once и переиспользуется во всех handler-middleware.
//
// Интеграция следует ADR-0010: каждое доменное событие (login, refresh,
// applicant.registered) должно попадать в append-only audit log сервиса
// audit-service для compliance и реконсиляции с биллингом.
package audit
...
// Client возвращает singleton audit-клиента.
...
// MustClient возвращает singleton client; никогда не падает: при
// ошибке возвращает nil, чтобы handler-middleware мог безопасно сгладить
// отсутствие audit-эмиттера в DEV-окружении.
