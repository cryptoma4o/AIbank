...
// tenant-service.  Lazy singleton: клиент инициализируется один раз через
// sync.Once и переиспользуется во всех handler-middleware.
//
// Интеграция следует ADR-0010: каждое доменное событие должно попадать в
// append-only audit log сервиса audit-service, чтобы биллинг и операционные
// отчёты могли по correlation_id связать audit-запись с биллинговой строкой.
package audit
...
// когда URL некорректен (но в DEV-режиме main вызывает ResetForTest, а
// сам init выполняется без аборта).
