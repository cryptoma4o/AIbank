...
func TestBuildProvider_LiveFlagReturnsLive(t *testing.T) {
	...
	t.Setenv("FSSP_LIVE_ENDPOINT", "https://example.invalid/fssp")
	p := BuildProvider()
	live, ok := p.(*LiveProvider)
	if !ok {
		t.Fatalf("ожидался *LiveProvider при FSSP_LIVE=true, получили %T", p)
	}
	if live.Name() != "live" {
		t.Fatalf("ожидалось имя 'live', получили %q", live.Name())
	}
	if live.cfg.Endpoint != "https://example.invalid/fssp" {
