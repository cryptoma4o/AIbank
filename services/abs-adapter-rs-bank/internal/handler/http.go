// Package handler — HTTP-обвязка адаптера RS-Bank/ЦАБС.
//
...
// настоящего интеграционного клиента к RS-Bank ещё нет (golden-tests +
// mock-RS-Bank — Phase 4, см. ADR-0006 § 5). Детерминизм нужен, чтобы тесты
// были стабильными и чтобы повторный вызов с тем же idempotency_key
// возвращал тот же результат (имитация идемпотентности на стороне адаптера).
package handler
...
const AdapterName = "rs-bank"

// AccountPrefix — фиксированный префикс «расчётный счёт юр.лица в рублях».
// RS-Bank-инстанс отличается reserved-меткой RSB в составе номера, чтобы
// в логах и в тестовых сценариях легко различать banking origin.
const AccountPrefix = "40702810"

// AccountReservedTag — метка, отделяющая rs-bank-номер от cft/diasoft-номера
// в stub-режиме. На реальном RS-Bank 20-значный номер будет считаться
// по алгоритму ЦБ (Положение 579-П), и эта метка исчезнет.
const AccountReservedTag = "RSB"

// BIK — пример БИК банка-владельца RS-Bank-инстанса (статический для stub'а).
const BIK = "044030653"

// Handler — HTTP-handler RS-Bank-адаптера. Stateless, безопасен для
...
func (h *Handler) stub(cmd domain.ABSCommand) domain.ABSResponse {
	...
	case domain.CmdOpenAccount:
		...
		// stub-маркер, отсутствующий в реальной RS-Bank-нумерации.
	...
	case domain.CmdCreateClient:
		base.Success = true
		base.Data = map[string]any{
			"client_id": "rsbank_" + digestHex(cmd, 16),
		}
...
func digestHex(cmd domain.ABSCommand, n int) string {
	sum := sha256.Sum256([]byte("rs-bank|" + string(cmd.Command) + "|" + cmd.IdempotencyKey))
...
// действительно содержит метку RSB в нужной позиции (после AccountPrefix).
