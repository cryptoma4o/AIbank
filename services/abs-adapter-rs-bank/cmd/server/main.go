// Package main — точка входа адаптера RS-Bank/ЦАБС (см. ADR-0006).
//
// Сервис слушает HTTP на :9003 и предоставляет:
//
...
// пакетом adapterrsbank. Это нужно, чтобы Docker-image-tag
// (`aibank/abs-adapter-rs-bank:<version>`) и значение в audit log
...
import (
	...
	adapterrsbank "aibank/abs-adapter-rs-bank"
	"aibank/abs-adapter-rs-bank/internal/handler"
)

func main() {
	version := strings.TrimSpace(adapterrsbank.Version())
	if version == "" {
		version = "0.0.0-dev"
	}
	...
	obsProvider, err := obs.Init(obsCtx, obs.Config{
		ServiceName: "abs-adapter-rs-bank",
	...
	obsCancel()
	if err != nil {
		log.Fatalf("abs-adapter-rs-bank: init observability: %v", err)
	}
	defer func() {
	...
	srv := &http.Server{
		Addr:         ":9003",
		Handler:      otelhttp.NewHandler(h.Router(), "abs-adapter-rs-bank"),
	...
	go func() {
		log.Printf("abs-adapter-rs-bank %s: listening on :9003", version)
