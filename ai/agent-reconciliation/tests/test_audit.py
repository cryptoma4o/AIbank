"""Audit-emission test: a successful /v1/reconcile publishes one event."""
...
def client_with_audit(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(reconciler_mod, "GatewayClient", lambda *a, **kw: fake_gw)
...
def test_reconcile_publishes_audit_event(client_with_audit) -> None:
    client, fake_audit = client_with_audit
    body = {
        "tenant_id": "bank-alpha",
        "application_id": "app-99",
        "extracted_data": {"director": "Иванов"},
        "egrul_data": {"director": "Иванов"},
    }
    resp = client.post(
        "/v1/reconcile", json=body, headers={"X-Actor-ID": "pipeline-1"}
    )
    ...
    assert req.event_type == "agent.reconciliation.invoked"
    assert req.entity_type == "reconciliation_run"
    assert req.entity_id == "app-99"
    assert req.actor_id == "pipeline-1"
