...
func TestExecute_OpenAccount_ReturnsAccountWithRsbTag(t *testing.T) {
	h := NewHandler("1.0.0").Router()
	cmd := domain.ABSCommand{
		IdempotencyKey: "open-account-001",
		TenantID:       "bank-gamma",
		Command:        domain.CmdOpenAccount,
		Payload:        map[string]any{"client_id": "rsbank_demo", "account_type": "current_rub"},
	}
	...
	if !HasReservedTag(acc) {
		t.Fatalf("expected RSB reserved tag in account_number, got %q", acc)
	}
	if len(acc) != 20 {
		t.Fatalf("expected 20-symbol account_number, got %d (%q)", len(acc), acc)
	}
	if resp.AdapterUsed != "abs-adapter-rs-bank" {
...
func TestExecute_CreateClient_PrefixRsbank(t *testing.T) {
	h := NewHandler("1.0.0").Router()
	cmd := domain.ABSCommand{
	...
	if !ok || !strings.HasPrefix(clientID, "rsbank_") {
		t.Fatalf("expected client_id with rsbank_ prefix, got %#v", resp.Data)
	}
}
...
func TestVersion_ReturnsAdapterMetadata(t *testing.T) {
	...
	if payload["adapter"] != "rs-bank" {
		t.Fatalf("expected adapter=rs-bank, got %v", payload["adapter"])
	}
	if payload["version"] != "2.0.1" {
		t.Fatalf("expected version=2.0.1, got %v", payload["version"])
	}
